...
@bot.event
async def on_ready():
    # Эта функция синхронизирует слеш-команды с Discord.
    # Если вы запускаете бота впервые, может потребоваться до часа,
    # чтобы команды стали видны.
    try:
        await bot.tree.sync()
...
if __name__ == "__main__":
    # Эта проверка гарантирует, что программа завершится, если токен не установлен.
    # Поскольку вы вставили токен, эта проверка теперь не нужна, но мы ее оставим.
    ...
    # Запуск бота. Если вы все еще видите ошибку 401 Unauthorized,
    # токен, который вы предоставили, недействителен.
    # В этом случае вам нужно СБРОСИТЬ его на портале разработчиков Discord.
    try:
        bot.run(BOT_TOKEN)
    except discord.errors.LoginFailure as e:
        print(f"\n🛑 КРИТИЧЕСКАЯ ОШИБКА ВХОДА: {e}")
        print("➡️ Пожалуйста, сбросьте токен на Портале разработчиков Discord и замените его в коде.")
